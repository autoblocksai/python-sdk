...
from autoblocks._impl.prompts.v2.client import V2APIClient
from autoblocks._impl.prompts.v2.models import Prompt
from autoblocks._impl.prompts.v2.utils import normalize_app_name, to_title_case, to_snake_case, indent, infer_type
...
def generate_factory_class_code(title_case_id: str, prompt_id: str, major_versions: List[str], app_id: str) -> str:
    ...
    auto += f"{indent(2)}raise ValueError(f\"Unsupported major version: {{major_version}}. Available versions: {available_versions}\")\n"
...
def generate_prompt_implementations(app_id: str, app_name: str, prompt_data: Prompt) -> str:
    ...
    for major_version in prompt_data.major_versions:
        ...
        params_class = generate_params_class_code(title_case_id, version, major_version.params.dict() if major_version.params else {})
        implementations.append(params_class)
        
        # Generate template renderer
        template_renderer = generate_template_renderer_class_code(title_case_id, version, [t.dict() for t in major_version.templates])
        implementations.append(template_renderer)
        
        # Generate tool renderer
        tool_renderer = generate_tool_renderer_class_code(title_case_id, version, [t.dict() for t in major_version.tools_params] if major_version.tools_params else [])
...
def generate_app_init(app_name: str, app_id: str, prompts: List[Prompt], output_dir: str) -> None:
    ...
    for prompt in prompts:
        ...
        init_code.append(f"                  api_key=None, init_timeout=None,")
        init_code.append(f"                  refresh_timeout=None, refresh_interval=None):")
        init_code.append(f"    return prompts.{to_title_case(prompt_id)}Factory.create(")
        init_code.append(f"        major_version=major_version,")
        init_code.append(f"        minor_version=minor_version,")
        init_code.append(f"        api_key=api_key,")
        init_code.append(f"        init_timeout=init_timeout,")
        init_code.append(f"        refresh_timeout=refresh_timeout,")
        init_code.append(f"        refresh_interval=refresh_interval")
        init_code.append(f"    )")
        init_code.append("")
    ...
    init_code.append(f"# Register app with registry")
    init_code.append(f"app = PromptRegistry.register_app(")
    ...
    for prompt in prompts:
        ...
        init_code.append(f"PromptRegistry.register_prompt(")
...
def generate_all_prompt_modules(api_key: Optional[str] = None, output_dir: str = None) -> None:
    """
    ...
    client = V2APIClient(api_key=api_key)
    ...
    for prompt in all_prompts:
        ...
        apps_map[app_id]["prompts"].append(prompt)
